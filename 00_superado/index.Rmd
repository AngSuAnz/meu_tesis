...
proyectar_punto_en_linea <- function(geom_linea, geom_punto) {
  linea_proxima <- st_nearest_points(geom_punto, geom_linea)

  # Si devuelve una colección, extraer el segundo punto
  if (inherits(linea_proxima, "GEOMETRYCOLLECTION")) {
    punto_proyectado <- st_collection_extract(linea_proxima, "POINT")[2]
  } else {
    punto_proyectado <- st_cast(linea_proxima, "POINT")[2]
  }
