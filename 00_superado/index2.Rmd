...
comercios_caba = opq(bbox_caba)

comercios_caba = add_osm_feature(comercios_caba, key = "building", value = c("shop", "supermarket")) %>% 
  osmdata_sf()


# INCLUIR TAMBIEN: key="shop", value=c("food","greengrocer","grocery", "supermarket")
comercios_caba = comercios_caba$osm_points

ggplot(comercios_caba) +
  geom_sf()


mercados_caba = st_read("data/mercados.shp")%>% 
                    st_transform(22185) %>% 
                    #cbind(st_drop_geometry(centroides)) %>% 
                    st_make_valid()
                    #mutate(id = row_number())
...
leaflet(st_zm(sectores_prox)) %>%
  ...
  addLayersControl(
    baseGroups = c("OSM","Satelite"), 
    overlayGroups = c("Barrio Mugica", "Sectores Proximos", "Comercios CABA"))%>% 
  ...
  addCircleMarkers(data=mercados_caba,
              fillColor = "black",
              weight = 0.5,
              color = "black",
              fillOpacity = 0.5,
              radius = 2,
              group = "Comercios CABA")
...
paradas_desde_bm = paradas_bm %>%
  unnest(linea_sentido) %>%      # Desanidamos la lista
  filter(linea_sentido!=" NANA") %>%   # Eliminamos NAs (si hay)
...
paradas_desde_bm <- paradas_desde_bm %>%
  mutate(geometry_parada = st_geometry(.))  # store geometry as a normal column
  
recorridos_desde_bm = left_join(paradas_desde_bm %>% 
                                  st_drop_geometry(), 
                                recorridos_bm 
                                  %>% select(linea_sentido,geometry_recorrido), 
                                by = "linea_sentido")

recorridos_desde_bm$geometry_parada <- st_sfc(recorridos_desde_bm$geometry_parada, crs = st_crs(paradas_bm))
